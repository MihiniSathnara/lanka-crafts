...
import { formatTime, getErrorMessage } from '../../utils/helpers.js';
import toast from 'react-hot-toast';

export default function ArtistChats() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const handler = ({ chatId, message }) => {
      if (activeChat?._id === chatId) setMessages(prev => [...prev, message]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setChats(prev => prev.map(c => c._id === activeChat._id ? { ...c, lastMessage: content } : c));
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Tourist Messages</h1>

      <div className="card flex h-[600px] overflow-hidden">
        <div className="w-72 border-r border-gray-100 flex flex-col flex-shrink-0">
          <div className="p-3 border-b border-gray-100">
            <h3 className="text-sm font-semibold text-gray-600">Tourists ({chats.length})</h3>
          </div>
          <div className="overflow-y-auto flex-1">
            {chats.length === 0 ? (
              <div className="p-4 text-center text-gray-400 text-sm">No messages yet from tourists.</div>
            ) : chats.map(chat => (
              <button key={chat._id} onClick={() => setActiveChat(chat)}
                ...
                <div className="flex items-center gap-2">
                  <img src={chat.tourist?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(chat.tourist?.name || 'T')}&background=3b82f6&color=fff`}
                    alt="" className="w-9 h-9 rounded-full object-cover flex-shrink-0" />
                  <div className="min-w-0">
                    <p className="font-medium text-sm text-gray-900 truncate">{chat.tourist?.name}</p>
          ...
          {!activeChat ? (
            ...
            <>
              <div className="p-3 border-b border-gray-100 flex items-center gap-3">
                <img src={activeChat.tourist?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(activeChat.tourist?.name || 'T')}&background=3b82f6&color=fff`}
                  alt="" className="w-8 h-8 rounded-full" />
                <span className="font-semibold text-gray-900">{activeChat.tourist?.name}</span>
              </div>
                ...
                {msgLoading ? <Spinner /> : messages.length === 0 ? (
                  ...
                  return (
                    <div key={msg._id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                      {!isMine && (
                        <img src={msg.sender?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(msg.sender?.name || 'T')}&background=3b82f6&color=fff`}
              ...
              <div className="p-3 border-t border-gray-100 flex gap-2">
                <input value={input} onChange={e => setInput(e.target.value)}
                  ...
                  placeholder="Type a message..." className="flex-1 input-field text-sm" />
